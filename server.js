...
app.get('/messages', requireLogin, (req, res) => {
  ...
  db.all(sql, params, (err, rows) => {
    ...
    console.log("MESSAGES:", rows); // 👈 تست مهم
    res.json(rows);
  });
});
...
app.post('/message', requireLogin, upload.single('file'), (req, res) => {
  ...
  // تبدیل همه حالت‌های اشتباه به NULL واقعی
  if (!receiverId || receiverId === 'null' || receiverId === 'undefined' || receiverId === '' || receiverId === '0') {
    receiverId = null;
  }
  ...
  if (!content && !filePath) {
    return res.status(400).send('پیام یا فایل باید ارسال شود');
  }

  db.run(
    'INSERT INTO messages (sender_id, sender, receiver_id, content, file_path) VALUES (?, ?, ?, ?, ?)',
    [sender_id, sender, receiverId, content, filePath],
    function (err) {
